...
class Processor(val initialGridAndProgram: GridAndProgram, config: CompilerConfiguration) {
        ...
        case Some(operation) =>
          operation._1 match {
            ...
            case _ =>
              ...
              if (checkEveryFrame)
                executeHelp(state, process(state, operation, successCheck), stepCount , post.headOption, post.drop(1), successCheck, checkEveryFrame)
              else
                executeHelp(state, process(state, operation, _ => false), stepCount , post.headOption, post.drop(1), successCheck, checkEveryFrame)

          }
        case _ =>
          if (post.isEmpty) {
            Stream(generateFinalFrame(state, successCheck(state.currentGrid)))
          }
          else
  ...
  private def generateFinalFrame(state : ProcessorState, isSuccess : Boolean) : Frame =
    Frame(Final, state.currentRegister, state.currentGrid, TraceTag(UserFunction(), -1), Some(state.currentGrid.getRobotLocation), None, isSuccess)
  ...
  private def executeHelp(state : ProcessorState, executed : Frame, stepCount : Int, maybeOperation : Option[(Operation, TraceTag)], post : Seq[(Operation, TraceTag)], successCheck : Grid => Boolean, checkEveryFrame : Boolean) = {
    if (executed.success)
      Stream(executed, generateFinalFrame(state, executed.success))
    else
      executed #:: execute(state, maybeOperation, post, stepCount + 1 , 0, successCheck, checkEveryFrame)
  }

  private def process(state: ProcessorState, operation: (Operation, TraceTag),
    ...
    operation._1 match {
      ...
      case PickUpItem =>
        ...
        Frame(operation._1, state.currentRegister, grid, operation._2, Some(state.currentGrid.getRobotLocation), change, successCheck(state.currentGrid))
      ...
      case SetItemDown =>
        state.currentRegister.pop() match {
          ...
          case None =>
            ...
            Frame(operation._1, state.currentRegister, state.currentGrid, operation._2, Some(state.currentGrid.getRobotLocation), None, successCheck(state.currentGrid))
        }
      ...
      case _ => Frame(operation._1, state.currentRegister, state.currentGrid, operation._2, None, None, successCheck(state.currentGrid))
    }
}
