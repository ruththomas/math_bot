...
class CompilerActor @Inject()(out: ActorRef, tokenId: TokenId)(
  ...
  private def createLastFrame(programState: ProgramState, frame: Frame) = {
    ...
    for {
      ...
      pathAndContinent <- levelControl.updateStats(tokenId, checkForSuccess(programState, frame))
    } yield {
      if (checkForSuccess(programState, frame)) {
        List(MathBotCompiler.ClientFrame.success(frame, pathAndContinent.path, pathAndContinent.builtContinent))
      } else {
        List(MathBotCompiler.ClientFrame.failure(frame, pathAndContinent.path, pathAndContinent.builtContinent))
      }
    }
  }
  ...
  private def createLastFromFromNothing() = {
    for {
      pathAndContinent <- levelControl.updateStats(tokenId, success = true)
    } yield {
      MathBotCompiler.ClientFrame(ClientRobotState(Point(0, 0), "0", List.empty[String]),
                                  "failure",
                                  Some(pathAndContinent.path),
                                  Some(pathAndContinent.builtContinent))
    }
  }
  ...
  def createCompile(): Receive = {
    ...
    case CompilerCreate(steps, problem) =>
      ...
      for {
        continent <- levelControl.getBuiltContinent(tokenId)
      } yield {
