...
import actors.CurrentStatsActor.{StatsDoneUpdating, UpdateStats}
import actors.messages._
import actors.messages.level.StatsAndContinent
...
import models.{CurrentStats, FuncToken, GridMap, Problem}
...
class CompilerActor @Inject()(out: ActorRef, tokenId: TokenId)(
  ...
  private def createLastFrame(programState: ProgramState, frame: Frame) = {
    ...
    for {
      ...
      statsAndContinent <- (levelActor ? LevelActor.UpdateStats(tokenId, checkForSuccess(programState, frame)))
        .mapTo[StatsAndContinent]
    } yield {
      if (checkForSuccess(programState, frame)) {
        List(MathBotCompiler.ClientFrame.success(frame, statsAndContinent.stats, statsAndContinent.builtContinent))
      } else {
        List(MathBotCompiler.ClientFrame.failure(frame, statsAndContinent.stats, statsAndContinent.builtContinent))
      }
    }
  }
  ...
  private def createLastFromFromNothing() = {
    for {
      statsAndContinent <- (levelActor ? LevelActor.UpdateStats(tokenId, success = true))
        .mapTo[StatsAndContinent]
    } yield {
      MathBotCompiler.ClientFrame(ClientRobotState(Point(0, 0), "0", List.empty[String]),
                                  "failure",
                                  Some(statsAndContinent.stats),
                                  Some(statsAndContinent.builtContinent))
    }
  }
...
object CompilerActor {
  def props(out: ActorRef,
            ...
            logger: MathBotLogger,
            config: CompilerConfiguration) =
    Props(new CompilerActor(out, tokenId)(playerTokenDAO, levelActor, logger, config))
}
