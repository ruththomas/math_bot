...
class CompilerActor @Inject()(out: ActorRef, tokenId: TokenId)(
  ...
  private def createLastFrame(programState: ProgramState, frame: Frame) = {
    ...
    for {
      ...
      statsAndContinent <- levelControl.updateStats(tokenId, checkForSuccess(programState, frame))
    } yield {
  ...
  private def createLastFromFromNothing() = {
    for {
      statsAndContinent <- levelControl.updateStats(tokenId, success = true)
    } yield {
  ...
  def createCompile(): Receive = {
    ...
    case CompilerCreate(steps, problem) =>
      ...
      for { // todo - change this to get StatsAndContinent
        statsAndContinent <- levelControl.getStatsAndContinent(tokenId)
      } yield {
        val stats = statsAndContinent.stats
        val continent = statsAndContinent.builtContinent
